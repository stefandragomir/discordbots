...
class Furion_Logger(object):
    ...
    def archive_log(self):
        ...
        _archive_path = os.path.join(os.path.split(self.path)[0],"furion_log_archive")
        
        if not os.path.exists(_archive_path):

            os.makedirs(_archive_path)

        _archive_path = os.path.join(_archive_path,strftime("furion_log_%d_%m_%Y_%H_%M_%S.zip", gmtime()))
